...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		LinkedList<Entry>[] tableTemp = new LinkedList[newTableSize];

		for (LinkedList<Entry> list : table) {
			if (list != null) {
				for (Entry entry : list) {
					int index = hash(entry.key.hashCode());
					if (tableTemp[index] == null) {
						tableTemp[index] = new LinkedList<Entry>();
					}
					tableTemp[index].add(entry);
				}
			}
		}
	...
	public void put(Object key, Object value){
		if(key==null) return;
		if ((float) numEntries / tableSize >= maxLoadFactor) rehash();
