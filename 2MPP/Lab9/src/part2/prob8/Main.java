...
public class Main {
	...
	private boolean findProduct(String prodName) {
		...
		return orderItems.stream().anyMatch(item -> Optional.ofNullable(item).map(OrderItem::getProduct).map(Product::getProductName).map(pName -> pName.equals(prodName)).orElse(false));
	}
