...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		System.out.println(list.stream().sorted((n1,n2) ->  Math.abs(n1) == Math.abs(n2) ? Integer.compare(n1,n2) : Integer.compare(Math.abs(n1), Math.abs(n2))).toList());
	}
	...
	public static void ordering2(List<String> words) {
		System.out.println(words.stream().sorted((s1,s2)-> new StringBuilder(s1).reverse().compareTo(new StringBuilder(s2).reverse())).toList());
	}
}
